...
class Event:
	def __init__(self, bseglist):
		if isinstance(bseglist, str): 
			...
			self.segs=[]
			self.uppercosts={} #[0]*self.numhists
			self.lowercosts={} #[0]*self.numhists
			self.dupsremoved=True
			
		elif isinstance(bseglist, list):
			self.id="%d.%d" % (bseglist[0].historyid, bseglist[0].order) #bseglist[0].ptrid
			self.likelihood=0
...
#			sys.stderr.write("border: %d, eventorder: %d, historyid: %d, histid: %d\n" % (braneyseg.order, eventorder, braneyseg.historyid, histid))
			if eventorder==-1: 
