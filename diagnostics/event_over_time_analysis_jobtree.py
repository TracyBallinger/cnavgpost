...
class Setup(Target): 
	...
	def run(self):
		...
		self.setFollowOnTarget(CombineEventCounts(outputdir, mylines))
...
class CombineEventCounts(Target):
	def __init__(self, outputdir, samples): 
		Target.__init__(self)
		self.outputdir=outputdir
	...
	def run(self):
		...
		for line in mylines:
			(cnavgout, sampleid) = line.strip().split('\t')[:2]
			datfiles=glob.glob(os.path.join(cnavgout, "lotadir", "*s.dat"))
			for datfile in datfiles:
				...
				if datkey in mydata.keys(): 
					...
					mydata[datkey]=dat
			Pcountfh=open(os.path.join(cnavgout, "lotadir", "true_count.txt"))
